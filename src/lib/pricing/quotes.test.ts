...
describe('restoreQuote', () => {
	...
	async function makeQuote(windowCount: number) {
		const { quote } = await createQuote(harness.env, {
			clientId: 'client-1',
			propertyId: 'property-1',
			input: {
				stories: 1,
				condition: 'light',
				counts: { ...ZERO_COUNTS, windowExtStandard: windowCount },
				hardWater: false,
				constructionDebris: false,
				difficultAccess: false,
			},
	...
	function itemArchivedAtValues(quoteId: string): (string | number | boolean | null)[] {
		const itemHeaders = harness.spreadsheet.getTab('QuoteItems')[0];
		const quoteIdIdx = itemHeaders.indexOf('Quote ID');
		const archivedAtIdx = itemHeaders.indexOf('Archived At');
		return harness.spreadsheet
			.getTab('QuoteItems')
			.slice(1)
			.filter((r) => r[quoteIdIdx] === quoteId)
			.map((r) => r[archivedAtIdx]);
	}

	function quoteArchivedAt(quoteId: string): string | number | boolean | null {
		const quoteHeaders = harness.spreadsheet.getTab('Quotes')[0];
		const quoteIdIdx = quoteHeaders.indexOf('Quote ID');
		const archivedAtIdx = quoteHeaders.indexOf('Archived At');
	...
	it('restoreQuote undoes deleteQuote — quote and its items un-archived', async () => {
		...
		await deleteQuote(harness.env, quote['Quote ID']);
		expect(quoteArchivedAt(quote['Quote ID'])).toBeTruthy();

		await restoreQuote(harness.env, quote['Quote ID']);

		expect(quoteArchivedAt(quote['Quote ID'])).toBeFalsy();
		const itemStates = itemArchivedAtValues(quote['Quote ID']);
		expect(itemStates.length).toBeGreaterThan(0);
		expect(itemStates.every((v) => !v)).toBe(true);
	});
